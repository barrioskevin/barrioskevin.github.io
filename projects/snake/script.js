...
const update = (delta) => {
	...
	*/
	...
	{
		if(Math.floor(snakex) === Math.floor(snakeBody.at(i).x) &&
	...
	if(Math.floor(snakex) < 0 || Math.floor(snakex) > 500 ||
		   Math.floor(snakey) < 0 || Math.floor(snakey) > 500)
	{
		gameover = true;
	}

	if(touching({"x": snakex, "y": snakey, "w": 10, "h": 10}, {"x": applex, "y": appley, "w": 10, "h": 10}))
	{
		score += 1;
